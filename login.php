	...
	<head>
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				if(htmlspecialchars(empty($name))) {
					$name_not_found = "burası boş olamaz.";
					$works = false;
				} 
				if(htmlspecialchars(empty($pass))) {
					$pass_not_found = "burası boş olamaz.";
					$works = false;
				} 
				if($works) {
					$existence = $db->Users("name = ? AND password = ?", $name, $pass)->fetch();
					if(empty($existence)) {
						$pass_not_found = "kullanıcı adı veya şifre yanlış.";
					}
					else {
						$_SESSION["username"] = $name;
						$_SESSION["id"] = $existence["usr_id"];
						$_SESSION["authority"] = (int) $existence["authority"];
						header('Location: /');
					}
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post"><input name="__RequestVerificationToken" type="hidden" />      <fieldset class="vertical">
        <input id="ReturnUrl" name="ReturnUrl" type="hidden" value="https://localhost/" />
        <span class="field-validation-valid" data-valmsg-for="Status" data-valmsg-replace="true"></span>
        <div>
          <label for="UserName">kullanıcı adı</label>
		  
          <input autofocus="autofocus" data-val="true" data-val-length="The field e-mail adresi must be a string with a maximum length of 50." data-val-length-max="50" data-val-required="kullanıcı adı boş olamaz" id="username" maxLength="130" name="Username" type="text" value="" />
          <?php echo ("<font color=\"red\">$name_not_found</font>") ?>
		  <span class="field-validation-valid" data-valmsg-for="UserName" data-valmsg-replace="true"></span>
        </div>
        <div>
          <label for="Password">şifre</label>
		  
          <input data-val="true" data-val-length="The field şifre must be a string with a maximum length of 50." data-val-length-max="50" data-val-required="şifre boş olamaz" id="password" maxLength="50" name="Password" type="password" />
         <?php echo ("<font color=\"red\">$pass_not_found</font>") ?>
		 <span class="field-validation-valid" data-valmsg-for="Password" data-valmsg-replace="true"></span>
        </div>
        <div>
          <!--<label class="checkbox">
            <input data-val="true" data-val-required="Remembering" id="RememberMe" name="RememberMe" type="checkbox" value="true" /><input name="RememberMe" type="hidden" value="false" />
            sonradan hatırla
          </label> -->
        </div>
        <div class="actions">
          <button type="submit" class="btn btn-primary btn-lg btn-block">girmeye çalış</button>
        </div>
      </fieldset>
</form>  <h1>giremeyiş</h1>
  <ul>
    <li><a href="/register.php">kayıtlı birisine dönüş</a></li>
  </ul>
