	...
	<body class="light-theme theme-disabled" itemscope itemtype="http://schema.org/WebPage">
        ...
        <div id="main">	
			...
			<div id="content" class="instapaper_body" role="main">
                <section id="content-body">
					...
					<?php
						...
						isRead = 1
						WHERE isRead = 0 AND receiver_id = " . $_SESSION["id"] ;
						$query = $pdo->prepare($sql);
						$query->execute();
						$works = true;
						$warning = $warning_user = $warning_msg = "";
						if ($_SERVER["REQUEST_METHOD"] == "POST") {
							
							if(isset($_POST['delete'])) {
								$_SESSION["delete"] = $_SESSION["delete"] + 1;
								$warning = "bir kez daha bu tuşa basarsan mesaj silinir.";
								if($_SESSION["delete"] === 2) {
									$_SESSION["delete"] = 0;
									$thisfetch = $db->messages("message_id = ?", $_POST['delete'])->fetch();
									if($thisfetch['receiver_id'] == $_SESSION['id']) {
										$sql = "UPDATE messages SET
										hidden_receiver = 1
										WHERE message_id = " . $_POST['delete'];
										$query = $pdo->prepare($sql);
										$query->execute();
										} else {
										$sql = "UPDATE messages SET
							...
							if(isset($_GET['sent'])) {
								echo "Mesaj başarıyla gönderildi efendimiz.";
							}
							if(isset($_POST['send'])) {
								
								$who = $db->users("name = ?", $_POST["receipt"])->fetch();
								if(empty($who)) {
									$warning_user = "uygunsuz bir giriş yaptın.";
									$works = false;
								}
								if(empty($_POST['msg'])) {
									$warning_msg = "mesaj kısmı boş olamaz.";
									$works = false;
								}
								if($works) {
									$findmax = $db->messages()->max("message_id");
									$txt = htmlspecialchars($_POST['msg']);
									$txt = mb_strtolower($txt, 'utf8');
									$txt = str_replace("\n", "<br>", $txt);
									$array = array(
									
									"message_id" 	 => $findmax + 1,
									"sender_id" 	 => $_SESSION['id'],
									"receiver_id"   => $who['usr_id'],
									"text" 	     => $txt,
									"post_date" => new DateTime("now", new DateTimeZone('Europe/Istanbul')),
									"isRead"  => 0
									);
									$db->messages()->insert($array);	
									if(!headers_sent()) {
										header("Location: /message.php?sent=true");
										exit();
									}
								}
						...
						$messages = $db->messages()->select("*")->where("(receiver_id = " . $_SESSION["id"] . " AND hidden_receiver = 0) OR (sender_id = " . $_SESSION["id"] . " AND hidden_sender = 0)")->order("post_date DESC");
						foreach($messages as $m => $msg) {
							$date = new DateTime($msg["post_date"]);
							$proper_format = $date->format('d.m.Y H:i');
							$s = $db->users("usr_id = ?", $msg["sender_id"])->fetch();
							$r = $db->users("usr_id = ?", $msg["receiver_id"])->fetch();
							echo '<div id="message-thread">
							<article class="outgoing" style="margin-left:1%">
							<p><a href=/user.php?u=' . $s['usr_id'] . '>'. $s["name"] . '</a> --> <a href=/user.php?u=' .  $r['usr_id'] . '>' . $r["name"] . '</a> | ' . $msg["text"] . '</p>
							<footer>
							<time datetime="">' . $proper_format . '</time>
							</footer>
							</article>
							<font style="color:red">'.$warning.'</font>
							<form action="/message.php" id="message-thread-form" method="post">
							<p>
							<button type="submit" class="delete" title="böyle mesajlaşma olmaz olsun be arkadaş" name="delete" value="'. $msg['message_id'] . '" style="margin-left:45%">sil</button>
							</p>
							</form> 
							</div>';
						}
						
					?><h2 id="cevapla">mesaj yaz birilerine</h2>
					<form autocomplete="off" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" id="messaging" method="post">
						
						<span></span>
						Kime: <div class="acomp"><input type="text" cols="1" data-val="true" name="receipt" id="input-receipt" rows="1"></div>
						<?php require "autocomplete_users.php" ?>
						<?php echo ("<br><font color=\"red\">$warning_user</font>") ?>
						<br>İçerik
						<textarea class="edittextbox with-helpers" cols="80" data-val="true" name="msg" style="resize:none" placeholder="" rows="20"></textarea>
						<?php echo ("<font color=\"red\">$warning_msg</font>") ?>
						
						
						<div>
							<button type="submit" name="send" value="true" class="primary">ateşle</button>
						</div>
					</form>
			...
			include "setheader.php";
				?>
			<aside id="aside" style="margin-top: 10px">
				<div class="ad-double-click ad-300x150 ad-banner ad-doubleclickwebflipflopclock ads" data-info="{&quot;NetworkIdentifier&quot;:&quot;/1024435/desktop_web_flipflop_clock&quot;,&quot;Slot&quot;:&quot;div-gpt-ad-1511442987015-0&quot;,&quot;NetworkCode&quot;:&quot;1024435&quot;,&quot;Target&quot;:&quot;desktop_web_flipflop_clock&quot;,&quot;IsLazyLoad&quot;:false,&quot;Size&quot;:[300,150],&quot;Name&quot;:&quot;DoubleClickWebFlipFlopClock&quot;,&quot;DisplayTypes&quot;:4}"></div>
				
				<div class="ad-double-click ad-300x250 ad-banner ad-doubleclickweb300x250sideziyaretci ads" data-info="{&quot;NetworkIdentifier&quot;:&quot;/1024435/desktop_web_side&quot;,&quot;Slot&quot;:&quot;div-gpt-ad-1507285907658-1&quot;,&quot;NetworkCode&quot;:&quot;1024435&quot;,&quot;Target&quot;:&quot;desktop_web_side&quot;,&quot;IsLazyLoad&quot;:false,&quot;Size&quot;:[300,250],&quot;Name&quot;:&quot;DoubleClickWeb300x250SideZiyaretci&quot;,&quot;DisplayTypes&quot;:4}"></div>
				...
				<div id="sticky-ad">
					<div class="ad-double-click ad-300x250 ad-banner ad-doubleclickweb300x250sidesticky ads" data-info="{&quot;NetworkIdentifier&quot;:&quot;/1024435/desktop_web_side_sticky&quot;,&quot;Slot&quot;:&quot;div-gpt-ad-1507285907658-2&quot;,&quot;NetworkCode&quot;:&quot;1024435&quot;,&quot;Target&quot;:&quot;desktop_web_side_sticky&quot;,&quot;IsLazyLoad&quot;:true,&quot;Size&quot;:[300,250],&quot;Name&quot;:&quot;DoubleClickWeb300x250SideSticky&quot;,&quot;DisplayTypes&quot;:4}"></div>
				</div>
				<div id="sticky-criteo">
