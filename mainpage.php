...
<?php include "notorm-master/NotORM.php";
	...
	<head>
		...
		<?php
			...
			if(isset($_GET['action'])) 
			{
				...
				$findmax = $db->likes()->max("like_id");
				$like = $db->likes("usr_id = ? AND entry_id = ?", "$id", "$actionon")->fetch();
				if(empty($findmax)) $findmax = 0;
				if($_GET['action'] === 'like') {		
					if(empty($like)) {
						$db->likes()->insert(array(
						"like_id" => $findmax + 1,
						"entry_id" => $_GET["actionon"],
						"usr_id" => $_SESSION["id"],
						"isDislike" => 0
						));
					}
					...
					{
						$sql = "UPDATE likes SET
						isDislike = 0 WHERE like_id = " .$like["like_id"];
				...
				if($_GET['action'] === 'dislike') {
					...
					{
						$sql = "UPDATE likes SET
						isDislike = 1 WHERE like_id = " .$like["like_id"];
						$query = $pdo->prepare($sql);
						$query->execute();
					}
	...
	<div id="main">
		...
		<?php 
			if(!empty($_SESSION['username'])) {
				if(isset($_GET['t'])) {
					$tid = $db->Topics("topic_id = ?", $_GET['t'])->fetch();
					if($tid["topic_id"] === $_GET['t']) {
						include ("add_entry.php");
					}
				}	
				if(isset($_GET['tn'])) {
					if(!empty($_GET['tn'])) {
						$tid = $db->Topics("name = ?", $_GET['tn'])->fetch();
						if($tid["name"] === $_GET['tn']) {
							include ("add_entry.php");
						}
						else
...
<div id="hidden-theme-colors">
<div id="hidden-box-color"></div>
<div id="hidden-link-color"></div>
<div id="hidden-muted-color"></div>
<div id="hidden-body-text-color"></div>
<div id="hidden-text-color"></div>
<div id="hidden-intopic-link-color"></div>
<div id="hidden-entry-footer-info-color"></div>
</div>

<div id="hidden-eksiseyler-embed-style" style="display:none;" data-style="white"></div>



<!-- Google Tag Manager (noscript) -->
<iframe src="https://www.googletagmanager.com/ns.html?id=GTM-WXV2Z47"
...
<div style="display:inline;">
    <img height="1" width="1" style="border-style:none;" alt="" src="//googleads.g.doubleclick.net/pagead/viewthroughconversion/878852058/?value=0&amp;guid=ON&amp;script=0" />
</div></body>
</html>