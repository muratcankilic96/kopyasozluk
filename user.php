...
<?php
	include "user_intro.php"
	?>

<div class="profile-stats-sections" id="profile-stats-sections">
  <ul class="tabs2" id="profile-stats-section-nav" style="position=relative">
    <li class="active">
      <a 
         data-pagenumber="1" class="tab-trigger">entry'ler</a>
  </ul>
<div class="tabs-content" id="profile-stats-section-content">
	...
	if(isset($_GET['u'])) 
	{
		...
		if($_SESSION["authority"] != 2) {
			$entries = $db->entries()->select("*")->where("(usr_id = $getvar AND isHidden = 0) OR (usr_id = $log AND isHidden = 1)")->order("post_date DESC");
			} else {
			$entries = $db->entries()->select("*")->where("usr_id = $getvar")->order("post_date DESC");
		}
	}
	if(empty($entries[0])) 
	{
			$message = "bu yazar hiçbir şey yazmamış!";
	
		echo "<li style=\"list-style-type:none;\" data-id=\"0\" data-author=\"username\" data-author-id=\"1\" data-flags=\"share report vote\">
		<div class=\"content\">
		$message
		</div>
		<footer>
	...
	$counter = 0;
		foreach($entries as $e => $entry) 
		{
			$counter++;
		    $edit_date = "";
			$topic_of_entry = $db->topics('topic_id = ?', $entry["topic_id"])->fetch();
			...
			if(($_SESSION["username"] === $user_of_entry["name"]) or $_SESSION["authority"] == 2) {
				...
				if(!isset($_GET["e"])) {
					echo "<font color=\"darkred\"><h1>$topic_of_entry[name]</h1></font><br>";
				}
  ...
  <a href="#" class="load-more-entries hidden" data-pagenumber="1">
    daha fazla göster
  </a>
</div>
